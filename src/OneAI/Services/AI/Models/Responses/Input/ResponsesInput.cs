...
public class ResponsesInput : IOpenAiModels.IModel, IOpenAiModels.IUser
{
    ...
    public bool IsMessageArray
    {
        get
        {
            if (InputCalculated is IList<ResponsesMessageInput>) return true;
    ...
    ///     auto: If the context of this response and previous ones exceeds the model's context window size, the model will
    ///     truncate the response to fit the context window by dropping input items in the middle of the conversation.
    ///     disabled (default): If a model response will exceed the context window size for a model, the request will fail with
    ///     a 400 error.
    /// </summary>
